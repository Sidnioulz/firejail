...
int syscall_check_list(const char *slist, void (*callback)(int)) {
	...
	if (slist == NULL || *slist == '\0') {
		exechelp_logerrv("firejail", FIREJAIL_ERROR, "Error: empty syscall lists are not allowed\n");
		return -1;
	}
	...
	while (*ptr != '\0') {
		if (islower(*ptr) || isdigit(*ptr) || *ptr == '_')
			...
			*ptr = '\0';
			...
			if (nr == -1)
				exechelp_logerrv("firejail", FIREJAIL_WARNING, "Error: syscall %s not found\n", start);
