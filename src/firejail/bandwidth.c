...
static void shm_create_bandwidth_file(pid_t pid) {
	...
	if (fp) {
		...
		exechelp_logerrv("firejail", FIREJAIL_ERROR, "Error: cannot create bandwidth file in /dev/shm/firejail directory\n");
		exit(1);
	}
...
void network_shm_set_file(pid_t pid) {
	...
	if (fp) {
		...
		exechelp_logerrv("firejail", FIREJAIL_ERROR, "Error: cannot create network map file in /dev/shm/firejail directory\n");
		exit(1);
	}
...
void shm_write_bandwidth_file(pid_t pid) {
	...
	if (fp) {
		...
		exechelp_logerrv("firejail", FIREJAIL_ERROR, "Error: cannot write bandwidht file %s\n", fname);
		exit(1);
	}
}
...
void bandwidth_pid(pid_t pid, const char *command, const char *dev, int down, int up) {
	...
	if (!comm) {
		exechelp_logerrv("firejail", FIREJAIL_ERROR, "Error: cannot find sandbox\n");
		exit(1);
	}
	...
	if (!cmd || strstr(cmd, "--net") == NULL) {
		exechelp_logerrv("firejail", FIREJAIL_ERROR, "Error: the sandbox doesn't use a new network namespace\n");
		exit(1);
	}
	free(cmd);
	...
	if (find_child(pid, &child) == -1) {
		exechelp_logerrv("firejail", FIREJAIL_ERROR, "Error: cannot join the network namespace\n");
		exit(1);
	}
	...
	if (dev) {
		...
		FILE *fp = fopen(fname, "r");
		if (!fp) {
			exechelp_logerrv("firejail", FIREJAIL_ERROR, "Error: cannot read network map file %s\n", fname);
			exit(1);
		}
