...
static void check_dir_or_file(const char *name) {
	...
	if (s.st_uid != uid || s.st_gid != gid) {
		exechelp_logerrv("firejail", FIREJAIL_ERROR, "Error: only files or directories created by the current user are allowed.\n");
		exit(1);
	}
...
void fs_check_home_list(void) {
	if (strstr(cfg.home_private_keep, "..")) {
		exechelp_logerrv("firejail", FIREJAIL_ERROR, "Error: invalid private-home list\n");
		exit(1);
	}
