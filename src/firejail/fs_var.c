...
void fs_var_log(void) {
	...
	if (is_dir("/var/log")) {
		...
		exechelp_logerrv("firejail", FIREJAIL_WARNING, "Warning: cannot mount tmpfs in top of /var/log\n");
}
...
void fs_var_lock(void) {
	...
	if (is_dir("/var/lock")) {
		...
		if (lnk) {
			...
			exechelp_logerrv("firejail", FIREJAIL_WARNING, "Warning: /var/lock not mounted\n");
...
void fs_var_tmp(void) {
	...
	if (stat("/var/tmp", &s) == 0) {
		...
		exechelp_logerrv("firejail", FIREJAIL_WARNING, "Warning: /var/tmp not mounted\n");
...
void fs_var_utmp(void) {
	...
	if (stat("/var/run/utmp", &s) == 0)
		...
		exechelp_logerrv("firejail", FIREJAIL_WARNING, "Warning: cannot find /var/run/utmp\n");
		return;
	}
