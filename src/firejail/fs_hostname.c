...
void fs_hostname(const char *hostname) {
	...
	if (stat("/etc/hostname", &s) == 0) {
		...
		if (!fp) {
			exechelp_logerrv("firejail", FIREJAIL_ERROR, "Error: cannot create %s\n", fhost);
			free(fhost);
			exit(1);
		}
		fprintf(fp, "%s\n", hostname);
	...
	if (stat("/etc/hosts", &s) == 0) {
		...
		if (!fp1) {
			exechelp_logerrv("firejail", FIREJAIL_ERROR, "Error: cannot open /etc/hosts\n");
			free(fhost);
			exit(1);
		}
...
void fs_resolvconf(void) {
	...
	if (stat("/etc/resolv.conf", &s) == 0) {
		...
		exechelp_logerrv("firejail", FIREJAIL_ERROR, "Error: cannot set DNS servers, /etc/resolv.conf file is missing\n");
		exit(1);
	}
}
