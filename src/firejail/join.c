...
static void extract_command(int argc, char **argv, int index) {
	...
	if (arg_doubledash == 0 && *argv[index] == '-') {
		exechelp_logerrv("firejail", FIREJAIL_ERROR, "Error: invalid option %s after --join\n", argv[index]);
		exit(1);
	}
...
static void extract_caps_seccomp(pid_t pid) {
	...
	if (!fp) {
		free(file);
		exechelp_logerrv("firejail", FIREJAIL_ERROR, "Error: cannot open stat file for process %u\n", pid);
		exit(1);
	}
